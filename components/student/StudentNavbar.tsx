...
export default function StudentNavbar({ user }: { user: any }) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-[#020617]/80 backdrop-blur-md">
        ...
        <div className="flex-1 flex justify-end items-center gap-3 shrink-0">
          ...
          <div className="relative group">
            <button className="flex items-center gap-2.5 hover:bg-white/5 p-1 pr-3 rounded-full border border-transparent hover:border-white/5 transition-all">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-600 to-sky-600 flex items-center justify-center text-xs font-bold text-white">
                {user?.photo ? (
            ...
            {/* Dropdown Menu */}
            <div className="absolute right-0 top-full mt-2 w-48 bg-[#0f172a] border border-white/10 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all origin-top-right">
      ...
      {/* Mobile Nav */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-white/5 bg-[#020617]">
